...
# this macro.
AC_DEFUN([MPIRSHIM_SETUP_CXX],[
    AC_REQUIRE([MPIRSHIM_SETUP_CXX_BANNER])

    _MPIRSHIM_SETUP_CXX_COMPILER

    _MPIRSHIM_CXX_CHECK_EXCEPTIONS

    MPIRSHIM_CXX_FIND_TEMPLATE_REPOSITORY
    MPIRSHIM_CXX_FIND_TEMPLATE_PARAMETERS
    MPIRSHIM_CHECK_IDENT([CXX], [CXXFLAGS], [cc], [C++])

    _MPIRSHIM_CXX_CHECK_BUILTIN

    _MPIRSHIM_CXX_CHECK_2D_CONST_CAST
...
AC_DEFUN([_MPIRSHIM_SETUP_CXX_COMPILER],[
    MPIRSHIM_VAR_SCOPE_PUSH(mpirshim_cxx_compiler_works)
    ...
    AC_REQUIRE([MPIRSHIM_PROG_CXX])
    BASECXX="`basename $CXX`"

    AS_IF([test "x$CXX" = "x"], [CXX=none])
    set dummy $CXX
    mpirshim_cxx_argv0=[$]2
    MPIRSHIM_WHICH([$mpirshim_cxx_argv0], [MPIRSHIM_CXX_ABSOLUTE])
    AS_IF([test "x$MPIRSHIM_CXX_ABSOLUTE" = "x"], [MPIRSHIM_CXX_ABSOLUTE=none])

    AC_DEFINE_UNQUOTED(MPIRSHIM_CXX, "$CXX", [MPIRSHIM underlying C++ compiler])
    AC_SUBST(MPIRSHIM_CXX_ABSOLUTE)
    ...
    # running).  Note that it is  a fatal error if the C++ compiler
    # does not work
    AS_IF([test "$CXX" = "none"],
          [mpirshim_cxx_compiler_works=no],
          [AS_IF([test "$mpirshim_cv_cxx_compiler_vendor" = "microsoft" ],
                 [mpirshim_cxx_compiler_works=yes],
                 [MPIRSHIM_CHECK_COMPILER_WORKS([C++], [#include <string>
],
                                            [std::string foo = "Hello, world"],
                                            [mpirshim_cxx_compiler_works=yes],
                                            [mpirshim_cxx_compiler_works=no])])])

    AS_IF([test "$mpirshim_cxx_compiler_works" = "yes"],
          [_MPIRSHIM_SETUP_CXX_COMPILER_BACKEND],
          [AC_MSG_WARN([Could not find functional C++ compiler])
           AC_MSG_ERROR([Cannot continue])])


    AS_IF([test "$WANT_MPI_CXX_SUPPORT" = "1"],
          [MPIRSHIM_CXX_COMPILER_VENDOR([mpirshim_cxx_vendor])])
    MPIRSHIM_VAR_SCOPE_POP
])

# _MPIRSHIM_SETUP_CXX_COMPILER_BACKEND()
# ----------------------------------
# Back end of _MPIRSHIM_SETUP_CXX_COMPILER_BACKEND()
AC_DEFUN([_MPIRSHIM_SETUP_CXX_COMPILER_BACKEND],[
    # Do we want code coverage
    if test "$WANT_COVERAGE" = "1" && test "$WANT_MPI_CXX_SUPPORT" = "1"; then
        if test "$mpirshim_cxx_vendor" = "gnu" ; then
            AC_MSG_WARN([$MPIRSHIM_COVERAGE_FLAGS has been added to CFLAGS (--enable-coverage)])
            WANT_DEBUG=1
            CXXFLAGS="${CXXFLAGS} $MPIRSHIM_COVERAGE_FLAGS"
        else
    ...
    MPIRSHIM_CXXFLAGS_BEFORE_PICKY="$CXXFLAGS"
    if test "$WANT_PICKY_COMPILER" = 1 && test "$mpirshim_cxx_vendor" = "gnu"; then
        ...
        AC_CACHE_CHECK([if $CXX supports -Wno-long-double],
                   [mpirshim_cv_cxx_wno_long_double],
                   [AC_TRY_COMPILE([], [],
        ...
        AC_LANG_POP(C++)
        if test "$mpirshim_cv_cxx_wno_long_double" = "yes" ; then
    ...
    if test "$GXX" = "yes"; then
        ...
        AC_CACHE_CHECK([if $CXX supports -finline-functions],
                   [mpirshim_cv_cxx_finline_functions],
                   [AC_TRY_COMPILE([], [],
                                   [mpirshim_cv_cxx_finline_functions="yes"],
                                   [mpirshim_cv_cxx_finline_functions="no"])])
        AC_LANG_POP(C++)
        if test "$mpirshim_cv_cxx_finline_functions" = "yes" ; then
...
EOF
    ...
    *hpux*)
        if test "$BASECXX" = "CC"; then
            AC_MSG_WARN([*** You will probably have problems compiling MPIR-SHIM])
            AC_MSG_WARN([*** with the HP-UX CC compiler.  You should])
    ...
    # config/mpirshim_ensure_contains_optflags.m4
    MPIRSHIM_ENSURE_CONTAINS_OPTFLAGS(["$CXXFLAGS"])
    ...
    MPIRSHIM_C_GET_ALIGNMENT(bool, MPIRSHIM_ALIGNMENT_CXX_BOOL)
    AC_LANG_POP(C++)
])
...
# bindings
AC_DEFUN([_MPIRSHIM_CXX_CHECK_EXCEPTIONS],[
    ...
    AS_IF([test "$WANT_MPI_CXX_SUPPORT" = "0"],
          ...
          [_MPIRSHIM_CXX_CHECK_EXCEPTIONS_BACKEND])

    AC_DEFINE_UNQUOTED(MPIRSHIM_HAVE_CXX_EXCEPTION_SUPPORT, $HAVE_CXX_EXCEPTIONS,
        [Whether or not we have compiled with C++ exceptions support])
])

# _MPIRSHIM_CXX_CHECK_EXCEPTIONS_BACKEND()
# ------------------------------------
# Back end of _MPIRSHIM_CXX_CHECK_EXCEPTIONS
AC_DEFUN([_MPIRSHIM_CXX_CHECK_EXCEPTIONS_BACKEND],[
    AC_MSG_RESULT([$ENABLE_CXX_EXCEPTIONS])
    if test "$ENABLE_CXX_EXCEPTIONS" = "yes"; then
        # config/cxx_have_exceptions.m4
        MPIRSHIM_CXX_HAVE_EXCEPTIONS
        # config/cxx_find_exception_flags.m4
        MPIRSHIM_CXX_FIND_EXCEPTION_FLAGS
        if test "$MPIRSHIM_CXX_EXCEPTIONS" = "1"; then
            ...
            CFLAGS="$CFLAGS $MPIRSHIM_CXX_EXCEPTIONS_CXXFLAGS"
            AC_LANG_SAVE
            AC_LANG_C
            ...
            CXXFLAGS="$CXXFLAGS $MPIRSHIM_CXX_EXCEPTIONS_CXXFLAGS"
            LDFLAGS="$LDFLAGS $MPIRSHIM_CXX_EXCEPTIONS_LDFLAGS"
...
AC_DEFUN([_MPIRSHIM_CXX_CHECK_BUILTIN],[
    MPIRSHIM_VAR_SCOPE_PUSH([have_cxx_builtin_expect have_cxx_builtin_prefetch])
    have_cxx_builtin_expect=0
    have_cxx_builtin_prefetch=0

    AS_IF([test "$WANT_MPI_CXX_SUPPORT" = "1"],
          [_MPIRSHIM_CXX_CHECK_BUILTIN_BACKEND])

    AC_DEFINE_UNQUOTED([MPIRSHIM_CXX_HAVE_BUILTIN_EXPECT],
                       [$have_cxx_builtin_expect],
                       [Whether C++ compiler supports __builtin_expect])
    AC_DEFINE_UNQUOTED([MPIRSHIM_CXX_HAVE_BUILTIN_PREFETCH],
...
# _MPIRSHIM_CXX_CHECK_BUILTIN_BACKEND
# -------------------------------
# Back end of _MPIRSHIM_CXX_CHECK_BUILTIN
AC_DEFUN([_MPIRSHIM_CXX_CHECK_BUILTIN_BACKEND],[

    # see if the C++ compiler supports __builtin_expect
    AC_LANG_PUSH(C++)
    AC_CACHE_CHECK([if $CXX supports __builtin_expect],
        [mpirshim_cv_cxx_supports___builtin_expect],
        [AC_TRY_LINK([],
          [void *ptr = (void*) 0;
           if (__builtin_expect (ptr != (void*) 0, 1)) return 0;],
          [mpirshim_cv_cxx_supports___builtin_expect="yes"],
          [mpirshim_cv_cxx_supports___builtin_expect="no"])])
    if test "$mpirshim_cv_cxx_supports___builtin_expect" = "yes" ; then
    ...
    AC_CACHE_CHECK([if $CXX supports __builtin_prefetch],
        [mpirshim_cv_cxx_supports___builtin_prefetch],
        [AC_TRY_LINK([],
          [int ptr;
           __builtin_prefetch(&ptr,0,0);],
          [mpirshim_cv_cxx_supports___builtin_prefetch="yes"],
          [mpirshim_cv_cxx_supports___builtin_prefetch="no"])])
    if test "$mpirshim_cv_cxx_supports___builtin_prefetch" = "yes" ; then
...
AC_DEFUN([_MPIRSHIM_CXX_CHECK_2D_CONST_CAST],[
    MPIRSHIM_VAR_SCOPE_PUSH([use_2d_const_cast])
    use_2d_const_cast=0

    AS_IF([test "$WANT_MPI_CXX_SUPPORT" = "1"],
          [_MPIRSHIM_CXX_CHECK_2D_CONST_CAST_BACKEND])

    AC_DEFINE_UNQUOTED([MPIRSHIM_CXX_SUPPORTS_2D_CONST_CAST],
                       [$use_2d_const_cast],
...
# _MPIRSHIM_CXX_CHECK_2D_CONST_CAST_BACKEND
# ---------------------------------
# Back end of _MPIRSHIM_CHECK_2D_CONST_CAST
AC_DEFUN([_MPIRSHIM_CXX_CHECK_2D_CONST_CAST_BACKEND],[
    # see if the compiler supports const_cast of 2-dimensional arrays
    AC_LANG_PUSH(C++)
    AC_CACHE_CHECK([if $CXX supports const_cast<> properly],
       [mpirshim_cv_cxx_supports_2d_const_cast],
...
int cast_test(const int ranges[][3]) {
  return non_const_func(const_cast<int(*)[3]>(ranges));
            ...
            [],
            [mpirshim_cv_cxx_supports_2d_const_cast="yes"],
            [mpirshim_cv_cxx_supports_2d_const_cast="no"])])
    if test "$mpirshim_cv_cxx_supports_2d_const_cast" = "yes" ; then
        use_2d_const_cast=1
    fi
    AC_LANG_POP(C++)
])
