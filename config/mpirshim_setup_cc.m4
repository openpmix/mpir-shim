...
AC_DEFUN([MPIRSHIM_CC_HELPER],[
    MPIRSHIM_VAR_SCOPE_PUSH([mpirshim_cc_helper_result])
    AC_MSG_CHECKING([$1])

    AC_LINK_IFELSE([AC_LANG_PROGRAM([$3],[$4])],
                   [$2=1
                    mpirshim_cc_helper_result=yes],
                   [$2=0
                    mpirshim_cc_helper_result=no])

    AC_MSG_RESULT([$mpirshim_cc_helper_result])
    MPIRSHIM_VAR_SCOPE_POP
])


AC_DEFUN([MPIRSHIM_PROG_CC_C11_HELPER],[
    MPIRSHIM_VAR_SCOPE_PUSH([mpirshim_prog_cc_c11_helper_CFLAGS_save])

    mpirshim_prog_cc_c11_helper_CFLAGS_save=$CFLAGS
    CFLAGS="$CFLAGS $1"

    MPIRSHIM_CC_HELPER([if $CC $1 supports C11 _Thread_local], [mpirshim_prog_cc_c11_helper__Thread_local_available],
                   [],[[static _Thread_local int  foo = 1;++foo;]])

    MPIRSHIM_CC_HELPER([if $CC $1 supports C11 atomic variables], [mpirshim_prog_cc_c11_helper_atomic_var_available],
                   [[#include <stdatomic.h>]], [[static atomic_long foo = 1;++foo;]])

    MPIRSHIM_CC_HELPER([if $CC $1 supports C11 _Atomic keyword], [mpirshim_prog_cc_c11_helper__Atomic_available],
                   [[#include <stdatomic.h>]],[[static _Atomic long foo = 1;++foo;]])

    MPIRSHIM_CC_HELPER([if $CC $1 supports C11 _Generic keyword], [mpirshim_prog_cc_c11_helper__Generic_available],
                   [[#define FOO(x) (_Generic (x, int: 1))]], [[static int x, y; y = FOO(x);]])

    MPIRSHIM_CC_HELPER([if $CC $1 supports C11 _Static_assert], [mpirshim_prog_cc_c11_helper__static_assert_available],
                   [[#include <stdint.h>]],[[_Static_assert(sizeof(int64_t) == 8, "WTH");]])

    MPIRSHIM_CC_HELPER([if $CC $1 supports C11 atomic_fetch_xor_explicit], [mpirshim_prog_cc_c11_helper_atomic_fetch_xor_explicit_available],
	           [[#include <stdatomic.h>
#include <stdint.h>]],[[_Atomic uint32_t a; uint32_t b; atomic_fetch_xor_explicit(&a, b, memory_order_relaxed);]])


    AS_IF([test $mpirshim_prog_cc_c11_helper__Thread_local_available -eq 1 && test $mpirshim_prog_cc_c11_helper_atomic_var_available -eq 1 && test $mpirshim_prog_cc_c11_helper_atomic_fetch_xor_explicit_available -eq 1],
          [$2],
          [$3])

    CFLAGS=$mpirshim_prog_cc_c11_helper_CFLAGS_save

    MPIRSHIM_VAR_SCOPE_POP
])

AC_DEFUN([MPIRSHIM_PROG_CC_C11],[
    MPIRSHIM_VAR_SCOPE_PUSH([mpirshim_prog_cc_c11_flags])
    if test -z "$mpirshim_cv_c11_supported" ; then
        mpirshim_cv_c11_supported=no
        mpirshim_cv_c11_flag_required=yes
...
#error "Without any CLI flags, this compiler does not support C11"
                          ...
                          [mpirshim_cv_c11_flag_required=no])

        AC_MSG_RESULT([$mpirshim_cv_c11_flag_required])

        if test "x$mpirshim_cv_c11_flag_required" = "xno" ; then
            AC_MSG_NOTICE([verifying $CC supports C11 without a flag])
            MPIRSHIM_PROG_CC_C11_HELPER([], [], [mpirshim_cv_c11_flag_required=yes])
        fi

        if test "x$mpirshim_cv_c11_flag_required" = "xyes" ; then
            mpirshim_prog_cc_c11_flags="-std=gnu11 -std=c11 -c11"

            AC_MSG_NOTICE([checking if $CC supports C11 with a flag])
            mpirshim_cv_c11_flag=
            for flag in $(echo $mpirshim_prog_cc_c11_flags | tr ' ' '\n') ; do
                MPIRSHIM_PROG_CC_C11_HELPER([$flag],[mpirshim_cv_c11_flag=$flag],[])
                if test "x$mpirshim_cv_c11_flag" != "x" ; then
                    CFLAGS="$CFLAGS $mpirshim_cv_c11_flag"
                    AC_MSG_NOTICE([using $flag to enable C11 support])
                    mpirshim_cv_c11_supported=yes
                    break
                fi
            done
        else
...
AC_DEFUN([MPIRSHIM_SETUP_CC],[
    ...
    AC_REQUIRE([_MPIRSHIM_START_SETUP_CC])
    AC_REQUIRE([_MPIRSHIM_PROG_CC])
    AC_REQUIRE([AM_PROG_CC_C_O])

    MPIRSHIM_VAR_SCOPE_PUSH([mpirshim_prog_cc_c11_helper__Thread_local_available mpirshim_prog_cc_c11_helper_atomic_var_available mpirshim_prog_cc_c11_helper__Atomic_available mpirshim_prog_cc_c11_helper__static_assert_available mpirshim_prog_cc_c11_helper__Generic_available mpirshim_prog_cc__thread_available mpirshim_prog_cc_c11_helper_atomic_fetch_xor_explicit_available])
    ...
    MPIRSHIM_PROG_CC_C11

    if test $mpirshim_cv_c11_supported = no ; then
        ...
        # changed
        MPIRSHIM_PROG_CC_C11_HELPER([],[],[])
    fi

    # Check if compiler support __thread
    MPIRSHIM_CC_HELPER([if $CC $1 supports __thread], [mpirshim_prog_cc__thread_available],
    ...
    AC_DEFINE_UNQUOTED([MPIRSHIM_C_HAVE__THREAD_LOCAL], [$mpirshim_prog_cc_c11_helper__Thread_local_available],
                       [Whether C compiler supports __Thread_local])

    AC_DEFINE_UNQUOTED([MPIRSHIM_C_HAVE_ATOMIC_CONV_VAR], [$mpirshim_prog_cc_c11_helper_atomic_var_available],
                       [Whether C compiler supports atomic convenience variables in stdatomic.h])

    AC_DEFINE_UNQUOTED([MPIRSHIM_C_HAVE__ATOMIC], [$mpirshim_prog_cc_c11_helper__Atomic_available],
                       [Whether C compiler supports __Atomic keyword])

    AC_DEFINE_UNQUOTED([MPIRSHIM_C_HAVE__GENERIC], [$mpirshim_prog_cc_c11_helper__Generic_available],
                       [Whether C compiler supports __Generic keyword])

    AC_DEFINE_UNQUOTED([MPIRSHIM_C_HAVE__STATIC_ASSERT], [$mpirshim_prog_cc_c11_helper__static_assert_available],
                       [Whether C compiler supports _Static_assert keyword])

    AC_DEFINE_UNQUOTED([MPIRSHIM_C_HAVE___THREAD], [$mpirshim_prog_cc__thread_available],
                       [Whether C compiler supports __thread])

    MPIRSHIM_C_COMPILER_VENDOR([mpirshim_c_vendor])
    ...
    AS_IF([test "$mpirshim_cv_c_compiler_vendor" != "ibm"],
...
# undef _GNU_SOURCE
    ...
    if test "$WANT_COVERAGE" = "1"; then
        if test "$mpirshim_c_vendor" = "gnu" ; then
            ...
            MPIRSHIM_COVERAGE_FLAGS=

            AC_CACHE_CHECK([if $CC supports --coverage],
                      [mpirshim_cv_cc_coverage],
                      [AC_TRY_COMPILE([], [],
                                      [mpirshim_cv_cc_coverage="yes"],
                                      [mpirshim_cv_cc_coverage="no"])])

            if test "$mpirshim_cv_cc_coverage" = "yes" ; then
                MPIRSHIM_COVERAGE_FLAGS="--coverage"
                CLEANFILES="*.gcno ${CLEANFILES}"
                CONFIG_CLEAN_FILES="*.gcda *.gcov ${CONFIG_CLEAN_FILES}"
            else
                MPIRSHIM_COVERAGE_FLAGS="-ftest-coverage -fprofile-arcs"
                CLEANFILES="*.bb *.bbg ${CLEANFILES}"
                CONFIG_CLEAN_FILES="*.da *.*.gcov ${CONFIG_CLEAN_FILES}"
            fi
            CFLAGS="$CFLAGS_orig $MPIRSHIM_COVERAGE_FLAGS"
            LDFLAGS="$LDFLAGS_orig $MPIRSHIM_COVERAGE_FLAGS"

            MPIRSHIM_FLAGS_UNIQ(CFLAGS)
            MPIRSHIM_FLAGS_UNIQ(LDFLAGS)
            AC_MSG_WARN([$MPIRSHIM_COVERAGE_FLAGS has been added to CFLAGS (--enable-coverage)])
    ...
    MPIRSHIM_CFLAGS_BEFORE_PICKY="$CFLAGS"
    ...
    if test $WANT_PICKY_COMPILER -eq 1; then
        ...
        AC_CACHE_CHECK([if $CC supports -Wno-long-double],
            [mpirshim_cv_cc_wno_long_double],
            [AC_TRY_COMPILE([], [],
                [
        ...
        if test "$mpirshim_cv_cc_wno_long_double" = "yes" ; then
    ...
    if test "$GCC" = "yes"; then
        ...
        AC_CACHE_CHECK([if $CC supports -finline-functions],
                   [mpirshim_cv_cc_finline_functions],
                   [AC_TRY_COMPILE([], [],
                                   [mpirshim_cv_cc_finline_functions="yes"
                                    if test -s conftest.err ; then
                                        for i in unused 'not supported' ; do
                                            if $GREP -iq "$i" conftest.err; then
                                                mpirshim_cv_cc_finline_functions="no"
                                                break;
                                            fi
                                        done
                                    fi
                                   ...
                                   [mpirshim_cv_cc_finline_functions="no"])])
        if test "$mpirshim_cv_cc_finline_functions" = "yes" ; then
        ...
        AC_CACHE_CHECK([if $CC supports -fno-strict-aliasing],
                   [mpirshim_cv_cc_fno_strict_aliasing],
                   [AC_TRY_COMPILE([], [],
                                   [mpirshim_cv_cc_fno_strict_aliasing="yes"],
                                   [mpirshim_cv_cc_fno_strict_aliasing="no"])])
        if test "$mpirshim_cv_cc_fno_strict_aliasing" = "yes" ; then
    ...
    if test ! -z "$RESTRICT_CFLAGS" ; then
        ...
        AC_CACHE_CHECK([if $CC supports $RESTRICT_CFLAGS],
                   [mpirshim_cv_cc_restrict_cflags],
                   [AC_TRY_COMPILE([], [],
                                   [mpirshim_cv_cc_restrict_cflags="yes"],
                                   [mpirshim_cv_cc_restrict_cflags="no"])])
        if test "$mpirshim_cv_cc_restrict_cflags" = "yes" ; then
            add=" $RESTRICT_CFLAGS"
        fi
    ...
    AC_CACHE_CHECK([if $CC supports __builtin_expect],
        [mpirshim_cv_cc_supports___builtin_expect],
        [AC_TRY_LINK([],
          [void *ptr = (void*) 0;
           if (__builtin_expect (ptr != (void*) 0, 1)) return 0;],
          [mpirshim_cv_cc_supports___builtin_expect="yes"],
          [mpirshim_cv_cc_supports___builtin_expect="no"])])
    if test "$mpirshim_cv_cc_supports___builtin_expect" = "yes" ; then
        have_cc_builtin_expect=1
    else
        have_cc_builtin_expect=0
    fi
    AC_DEFINE_UNQUOTED([MPIRSHIM_C_HAVE_BUILTIN_EXPECT], [$have_cc_builtin_expect],
        [Whether C compiler supports __builtin_expect])

    # see if the C compiler supports __builtin_prefetch
    AC_CACHE_CHECK([if $CC supports __builtin_prefetch],
        [mpirshim_cv_cc_supports___builtin_prefetch],
        [AC_TRY_LINK([],
          [int ptr;
           __builtin_prefetch(&ptr,0,0);],
          [mpirshim_cv_cc_supports___builtin_prefetch="yes"],
          [mpirshim_cv_cc_supports___builtin_prefetch="no"])])
    if test "$mpirshim_cv_cc_supports___builtin_prefetch" = "yes" ; then
        have_cc_builtin_prefetch=1
    else
        have_cc_builtin_prefetch=0
    fi
    AC_DEFINE_UNQUOTED([MPIRSHIM_C_HAVE_BUILTIN_PREFETCH], [$have_cc_builtin_prefetch],
        [Whether C compiler supports __builtin_prefetch])

    # see if the C compiler supports __builtin_clz
    AC_CACHE_CHECK([if $CC supports __builtin_clz],
        [mpirshim_cv_cc_supports___builtin_clz],
        [AC_TRY_LINK([],
            [int value = 0xffff; /* we know we have 16 bits set */
             if ((8*sizeof(int)-16) != __builtin_clz(value)) return 0;],
            [mpirshim_cv_cc_supports___builtin_clz="yes"],
            [mpirshim_cv_cc_supports___builtin_clz="no"])])
    if test "$mpirshim_cv_cc_supports___builtin_clz" = "yes" ; then
        have_cc_builtin_clz=1
    else
        have_cc_builtin_clz=0
    fi
    AC_DEFINE_UNQUOTED([MPIRSHIM_C_HAVE_BUILTIN_CLZ], [$have_cc_builtin_clz],
    ...
    MPIRSHIM_ENSURE_CONTAINS_OPTFLAGS("$MPIRSHIM_CFLAGS_BEFORE_PICKY")
    MPIRSHIM_CFLAGS_BEFORE_PICKY="$co_result"

    AC_MSG_CHECKING([for C optimization flags])
    MPIRSHIM_ENSURE_CONTAINS_OPTFLAGS(["$CFLAGS"])
...
AC_DEFUN([_MPIRSHIM_START_SETUP_CC],[
    mpirshim_show_subtitle "C compiler and preprocessor"
...
AC_DEFUN([_MPIRSHIM_PROG_CC],[
    #
    # Check for the compiler
    #
    MPIRSHIM_VAR_SCOPE_PUSH([mpirshim_cflags_save dummy mpirshim_cc_arvgv0])
    mpirshim_cflags_save="$CFLAGS"
    AC_PROG_CC
    BASECC="`basename $CC`"
    CFLAGS="$mpirshim_cflags_save"
    AC_DEFINE_UNQUOTED(MPIRSHIM_CC, "$CC", [OMPI underlying C compiler])
    set dummy $CC
    mpirshim_cc_argv0=[$]2
    MPIRSHIM_WHICH([$mpirshim_cc_argv0], [MPIRSHIM_CC_ABSOLUTE])
    AC_SUBST(MPIRSHIM_CC_ABSOLUTE)
    MPIRSHIM_VAR_SCOPE_POP
])
