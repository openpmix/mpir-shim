...
AC_DEFUN([MPIRSHIM_CASE_SENSITIVE_FS_SETUP],[
#
# Arguments: none
#
# Dependencies: None
...
# (like HFS+ on MacOS X and MS Windows) are not case sensitive - mpirshimcc
# and mpirshimCC are the same file.
#
# Sets prefix_OS_HAVE_CASE_SENSITIVE_FS to 1 if filesystem is case
# sensitive (ie, mpirshimcc and mpirshimCC will be different files) or 0 if
...
if test "$with_cs_fs" = "yes"; then
    MPIRSHIM_WANT_CS_FS=1
elif test -z "$with_cs_fs"; then
    MPIRSHIM_WANT_CS_FS=$have_cs_fs
else
    MPIRSHIM_WANT_CS_FS=0
fi

if test "$MPIRSHIM_WANT_CS_FS" = "1"; then
    AC_MSG_RESULT([yes])
else
    AC_MSG_RESULT([no])
fi

AM_CONDITIONAL(CASE_SENSITIVE_FS, test "$MPIRSHIM_WANT_CS_FS" = "1")

if test "$MPIRSHIM_WANT_CS_FS" = "0"; then
	cat <<EOF
